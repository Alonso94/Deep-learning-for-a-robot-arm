...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        if len(action) == 14:
            for i in range(7):
                if action[2 * i] < action[2 * i + 1] and (self.a[i] + 0.2) < np.pi/2:
                    self.a[i] += 0.2
                if action[2 * i] >= action[2 * i + 1] and (self.a[i] - 0.2) > -np.pi/2:
                    self.a[i] -= 0.2
                vrep.simxSetJointPosition(self.ID, self.h[i], self.a[i], vrep.simx_opmode_oneshot)
        ...
        if len(cnts2) == 2:
            ...
            self.d3 = self.od3
        s = np.hstack(
            [self.a[0], self.a[1], self.a[2], self.a[3], self.a[4], self.a[5], self.a[6] , self.x1, self.y1,
